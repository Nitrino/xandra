defmodule BatchTest do
  ...
  test "batch of type \"logged\"", %{conn: conn} do
    ...
    assert {:ok, %Xandra.Batch{}, %Void{}} = Xandra.execute(conn, batch)

    {:ok, %Xandra.Simple{}, result} = Xandra.execute(conn, "SELECT name FROM users")
  ...
  test "using a default timestamp for the batch", %{conn: conn} do
    ...
    assert {:ok, %Xandra.Batch{}, %Void{}} = Xandra.execute(conn, batch, timestamp: timestamp)
  ...
  test "empty batch", %{conn: conn} do
    assert {:ok, %Xandra.Batch{}, %Void{}} = Xandra.execute(conn, Batch.new())
  end
