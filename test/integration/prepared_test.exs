defmodule PreparedTest do
  ...
  test "prepared functionality", %{conn: conn} do
    ...
    assert {:ok, %Xandra.Prepared{}, page} = Xandra.execute(conn, prepared, [1])
    ...
    assert {:ok, %Xandra.Prepared{}, page} = Xandra.execute(conn, prepared, %{"code" => 2})

    assert Enum.to_list(page) == [
             %{"name" => "Moe"}
           ]

    assert {:ok, %Xandra.Prepared{}, page} = Xandra.execute(conn, prepared, %{"code" => 5})
    assert Enum.to_list(page) == []
  end
  ...
  test "dynamic result columns", %{conn: conn} do
    ...
    assert {:ok, %Xandra.Prepared{}, page} = Xandra.execute(conn, prepared)
