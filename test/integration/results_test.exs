defmodule ResultsTest do
  ...
  test "each possible result", %{conn: conn, keyspace: keyspace} do
    assert {:ok, %Xandra.Simple{}, result} = Xandra.execute(conn, "USE #{keyspace}")
    assert result == %SetKeyspace{keyspace: String.downcase(keyspace)}

    statement = "CREATE TABLE numbers (figure int PRIMARY KEY)"
    assert {:ok, %Xandra.Simple{}, result} = Xandra.execute(conn, statement)
    ...
    assert {:ok, %Xandra.Simple{}, result} = Xandra.execute(conn, statement, [{"int", 123}])
    assert Enum.to_list(result) == [%{"figure" => 123}]

    assert {:ok, %Xandra.Simple{}, result} = Xandra.execute(conn, statement, [{"int", 321}])
    assert Enum.to_list(result) == []

    statement = "SELECT * FROM numbers WHERE figure = :figure"
    assert {:ok, %Xandra.Simple{}, result} = Xandra.execute(conn, statement, %{"figure" => {"int", 123}})
    assert Enum.to_list(result) == [%{"figure" => 123}]
  end
