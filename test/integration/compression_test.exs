defmodule CompressionTest do
  ...
  test "compression with the snappy algorithm", %{keyspace: keyspace} do
    assert {:ok, compressed_conn} = Xandra.start_link(compressor: Snappy, idle_interval: 200)
    ...
    assert {:ok, %Xandra.Simple{}, %Xandra.Page{} = page} = Xandra.execute(compressed_conn, statement, [{"int", 1}])
    ...
    assert {:ok, %Xandra.Prepared{}, %Xandra.Page{} = page} = Xandra.execute(compressed_conn, prepared, [1], options)
    assert Enum.to_list(page) == [%{"code" => 1, "name" => "Homer"}]

    # This sleep is needed to test pings with compression,
    # and its value must be bigger than :idle_interval.
    Process.sleep(250)
    ...
    assert {:ok, %Xandra.Batch{}, %Xandra.Void{}} = Xandra.execute(compressed_conn, batch, options)
  end
end
